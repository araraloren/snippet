...
pub struct Plugins {
    ...
    pub language: Vec<PathBuf>,
}

pub async fn collect_plugins(a: &Path) -> tokio::io::Result<Plugins> {
    let mut plugins = Plugins::default();
    let mut read_dir = read_dir(a).await?;
    ...
    while let Some(dir_entry) = read_dir.next_entry().await? {
        ...
        if let Some(name) = dir_entry.file_name().to_str() {
            if let Some(ext) = path.extension().and_then(|v| v.to_str()) {
                // only accept wasm
                if ext == "wasm" {
                    let parts: Vec<_> = name.split(|v| v == '_' || v == '-').collect();

                    if let Some([prefix, ty]) = parts.get(0..2) {
                        if *prefix == "snippet" {
                            if *ty == "compiler" {
                                plugins.compiler.push(path);
                            } else if *ty == "language" {
                                plugins.language.push(path);
                            }
                        }
                    }
                }
